...
float px;//泪心 github tearhacker  电报 t.me/TearGame
float py;
...
{
	FTransform1 Result;
	Result.Rotation.X = 读取浮点数(address);	// Rotation_X 
	Result.Rotation.Y = 读取浮点数(address + 4);	// Rotation_y
	Result.Rotation.Z = 读取浮点数(address + 8);	// Rotation_z
	Result.Rotation.W = 读取浮点数(address + 12);	// Rotation_w
	Result.Translation.X = 读取浮点数(address + 16);	// /Translation_X
	Result.Translation.Y = 读取浮点数(address + 20);	// Translation_y
	Result.Translation.Z = 读取浮点数(address + 24);	// Translation_z
	Result.Scale3D.X = 读取浮点数(address + 32);;	// Scale_X
	Result.Scale3D.Y = 读取浮点数(address + 36);;	// Scale_y
	Result.Scale3D.Z = 读取浮点数(address + 40);;	// Scale_z
	return Result;
}
